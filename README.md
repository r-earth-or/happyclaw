...
| `memory_get` | 读取记忆文件内容 |
| `set_env_var` | 持久化保存环境变量（与 Web 环境变量设置同源，重启后生效） |
...
4. **若使用飞书 SSO 登录**，在应用的「安全设置 → 重定向 URL」中添加：
   ```
   https://your-domain/api/auth/feishu/callback
   ```
   （本地开发填 `http://localhost:3000/api/auth/feishu/callback`；反向代理场景需在飞书配置中填写可公网访问的地址）
5. 发布应用版本并等待审批通过
6. 在 HappyClaw Web 界面的「设置 → IM 通道 → 飞书」中填入 App ID 和 App Secret

每个用户可在个人设置中独立配置飞书应用凭据，实现 per-user 的飞书 Bot。

管理员也可配置一个共享飞书机器人，通过发送者 `open_id` 自动路由到对应用户的工作区（多租户模式）。飞书账号可直接 SSO 登录，首次登录时自动创建用户账号（无密码）。

已有账号的用户可在「设置 → 个人资料 → 飞书账号」中绑定或解绑飞书账号。飞书 SSO 注册的用户可在不输入旧密码的情况下直接设置密码。
