...
- **MCP Server**：13 个工具（`send_message`、`schedule_task`、`list/pause/resume/cancel_task`、`register_group`、`install_skill`、`uninstall_skill`、`memory_append`、`memory_search`、`memory_get`、`set_env_var`），通过 SDK `createSdkMcpServer()` 以同进程模式注册
...
| `mcp-tools.ts` | MCP 工具定义：13 个工具通过 SDK `tool()` 注册，IPC 文件通信 |
...
- `POST /api/auth/register` · `PUT /api/auth/profile` · `PUT /api/auth/password`（飞书注册账号无密码时，`current_password` 可省略）
- `GET /api/auth/feishu/authorize` — 发起飞书 OAuth 授权跳转（返回 `authorizeUrl`）
- `GET /api/auth/feishu/bind-authorize` — 已登录用户绑定飞书账号（需认证，返回 `authorizeUrl`）
- `GET /api/auth/feishu/callback` — 飞书 OAuth 回调（需在飞书开放平台「安全设置 → 重定向 URL」中注册，格式：`https://your-domain/api/auth/feishu/callback`）
- `POST /api/auth/feishu/unbind` — 解绑飞书账号（需已设置密码，否则拒绝）
...
- **绑定**：调用 `GET /api/auth/feishu/bind-authorize` 获取授权链接，完成 OAuth 后回调携带 `?feishu_bind=success`（成功）或 `?feishu_bind=conflict`（open_id 已被其他账号占用）
